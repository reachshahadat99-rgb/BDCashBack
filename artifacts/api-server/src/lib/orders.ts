...
// Alias for the transaction argument type
 ...
 */
async function ensureWalletSnapshot(tx: Tx, userId: string) {
  // Upsert: insert if not present, otherwise do nothing and return existing
  await tx
...
// ---------------------------------------------------------------------------
// Cart (no financial writes, transactions not critical here)
// ---------------------------------------------------------------------------
...
export async function cancelOrder(userId: string, orderId: string) {
  // Use a transaction to atomically check status and update
  const result = await db.transaction(async (tx) => {
    const now = new Date();
    ...
    if (updated.length === 0) {
      // Check whether the order exists at all to give the right error
    ...
    // Restore stock for each item (unconditional add-back is safe: we only
    // cancel when the order was never shipped, so stock was never delivered)
    ...
    // Cancel each associated merchant order (only if still in a pre-fulfilment state)
...
// ---------------------------------------------------------------------------
// Withdrawals — fully atomic with race-safe deduction
// ---------------------------------------------------------------------------
...
export async function requestWithdrawal(
  ...
  const result = await db.transaction(async (tx) => {
    ...
    // Uses a WHERE predicate to prevent race-condition over-withdrawal
